...
const DEMO_JOBS = [
  { id:'j1', title:'Software Engineer', description:'Join TCS as a Software Engineer. You will work on enterprise applications and client projects across various technologies.', min_cgpa:6.0, eligible_branches:['CST','IT','ENC'], required_skills:['Java','SQL','Problem Solving'], package_lpa:7.0, job_type:'full-time', deadline:'2024-12-15', drive_date:'2024-12-20', max_backlogs:1, status:'active', companies:{id:'c1',name:'TCS',logo_url:null} },
  { id:'j2', title:'Systems Engineer', description:'Infosys is hiring fresh graduates for the Systems Engineer role. Training provided for 3 months at Mysore campus.', min_cgpa:6.5, eligible_branches:['CST','IT','ENC','AI'], required_skills:['C++','Python','Communication'], package_lpa:5.0, job_type:'full-time', deadline:'2024-12-10', drive_date:'2024-12-18', max_backlogs:0, status:'active', companies:{id:'c2',name:'Infosys',logo_url:null} },
  { id:'j3', title:'Software Developer', description:'Wipro is looking for passionate developers to join their Digital Business unit. Work on cutting-edge technology projects.', min_cgpa:7.0, eligible_branches:['CST','IT'], required_skills:['React.js','Node.js','MongoDB'], package_lpa:8.0, job_type:'full-time', deadline:'2024-12-20', drive_date:'2024-12-28', max_backlogs:0, status:'active', companies:{id:'c3',name:'Wipro',logo_url:null} },
  { id:'j4', title:'Associate Software Engineer', description:'Amazon is hiring for their operations tech team. Strong DSA skills required. 2 coding rounds + HR.', min_cgpa:7.5, eligible_branches:['CST','IT'], required_skills:['DSA','Python','System Design'], package_lpa:24.0, job_type:'full-time', deadline:'2024-11-30', drive_date:'2024-12-05', max_backlogs:0, status:'active', companies:{id:'c4',name:'Amazon',logo_url:null} },
];
...
export default function Jobs() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const apps = Array.isArray(a.data) ? a.data : (a.data?.applications||[]);
      setMyApps(apps.map(ap=>ap.job_id||ap.job_postings?.id).filter(Boolean));
  ...
  return (
    <div style={{padding:24,maxWidth:1000,margin:'0 auto'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:24}}>
        <div>
          <h1 style={{margin:0,fontSize:24,fontWeight:800,color:'var(--text-primary)',fontFamily:"'Sora',sans-serif"}}>Job Openings</h1>
      ...
      <div style={{display:'flex',gap:12,marginBottom:20,flexWrap:'wrap'}}>
        <div style={{display:'flex',alignItems:'center',gap:8,background:'var(--bg-input)',border:'1px solid var(--border)',borderRadius:9,padding:'8px 12px',flex:1,maxWidth:320}}>
          <Search size={15} color="#475569"/>
          <input style={{background:'none',border:'none',outline:'none',fontSize:13,color:'var(--text-primary)',flex:1,fontFamily:"'Sora',sans-serif"}} placeholder="Search company or role..." value={search} onChange={e=>setSearch(e.target.value)}/>
        </div>
          ...
          return (
            <GlowCard key={job.id} accent={isApplied?GREEN:CYAN}>
              <div style={{display:'flex',gap:14,alignItems:'flex-start'}}>
                {/* Logo */}
                <div style={{width:48,height:48,borderRadius:12,background:isApplied?'rgba(16,201,138,0.12)':'rgba(79,70,229,0.1)',border:`1px solid ${isApplied?GREEN:CYAN}33`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,marginBottom:6}}>
                    <div>
                      <p style={{margin:'0 0 2px',fontSize:16,fontWeight:700,color:'var(--text-primary)'}}>{job.title}</p>
                  ...
                  <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:8}}>
                    ...
                    {job.max_backlogs===0&&<span style={{...TAG,color:'#dc2626',background:'rgba(220,38,38,0.08)',border:'1px solid rgba(220,38,38,0.2)'}}>No backlogs</span>}
                  ...
                  <div style={{display:'flex',gap:10,alignItems:'center'}}>
                    ...
                    {!isApplied && (
                      <button style={{marginLeft:'auto',padding:'8px 20px',background:CYAN,color:'#ffffff',border:'none',borderRadius:8,fontSize:13,fontWeight:700,cursor:'pointer',fontFamily:"'Sora',sans-serif",display:'flex',alignItems:'center',gap:6}} onClick={()=>apply(job.id)} disabled={!!applying}>
...
const TAG = {display:'inline-flex',alignItems:'center',gap:4,padding:'2px 8px',background:'var(--bg-card-high)',border:'1px solid var(--border)',borderRadius:6,fontSize:11,color:'var(--text-secondary)',fontWeight:600};
const SEL = {background:'var(--bg-input)',border:'1px solid var(--border)',borderRadius:9,padding:'8px 12px',fontSize:13,color:'var(--text-primary)',outline:'none',fontFamily:"'Sora',sans-serif",cursor:'pointer'};
